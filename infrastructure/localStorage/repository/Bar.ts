...
export class BarRepository implements IBarRepository {
    ...
    async getOne(query?: BarOneQuery): Promise<Bar> {
        if (query) {
            const resp = BarRepository.repository.find(b => b.id === query.value);
            if (resp) {
                return resp;
            }
            throw new Error("404");
    ...
    async create(dto: CreateBarDto): Promise<Bar> {
        const bar = new Bar(v4(),dto.name, [], new Date());
        BarRepository.repository.push(bar);
        return bar;
    }
    async update(bar: Bar): Promise<void> {
        const barIndex = BarRepository.repository.findIndex(b => b.id === bar.id);
