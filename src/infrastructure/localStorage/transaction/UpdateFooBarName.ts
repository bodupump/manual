...
export class UpdateFooBarNameTransaction implements IUpdateFooBarNameTransaction {
    ...
    async update (fooId : string, barId : string, barName : string, fooName : string) : Promise<void> {
        const lastFoo = await this.fooRepository.getOne({target : FooTarget.id, value : fooId});
        const lastBar = await this.barRepository.getOne({target : BarTarget.id, value : barId});
        try {
            const newFoo = new Foo(lastFoo.id, fooName, lastFoo.bar);
            await this.fooRepository.update(newFoo);
            const newBar = new Bar(lastBar.id, barName, lastBar.foos, lastBar.date);
